...
class ProductView(View):
    def get(self, request):
        ...
        return render(request, 'app/home.html',
                      {'topwears': topwears, 'bottomwears': bottomwears, 'mobiles': mobiles, 'leptop': leptop})
...
def leptop(request, data=None):
    if data == None:
        ...
        leptop = Product.objects.filter(category='L').filter(discounted_price__lt=40000)
    elif data == 'above':
        leptop = Product.objects.filter(category='L').filter(discounted_price__gt=40000)
    return render(request, 'app/leptop.html', {'leptop': leptop})


def topwear(request, data=None):
    if data == None:
        topwear = Product.objects.filter(category='TW')
    elif data == 'Yellow' or data == 'Lee':
...
def checkout(request):
    ...
    if cart_product:
        for p in cart_product:
            tempamount = (p.quantity * p.product.discounted_price)
            amount += tempamount
        totalamont = amount + shipping_amount
    return render(request, 'app/checkout.html', {'add': add, 'totalamont': totalamont, 'cart_items': cart_items})
